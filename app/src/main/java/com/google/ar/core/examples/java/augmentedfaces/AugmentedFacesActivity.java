...
public class AugmentedFacesActivity extends AppCompatActivity implements GLSurfaceView.Renderer {
  ...
  public void onSurfaceCreated(GL10 gl, EGLConfig config) {
    ...
    try {
      ...
      noseObject.createOnGlThread(/*context=*/ this, "models/specs.obj", "models/specs.png");
...
//        noseObject.draw(viewMatrix, projectionMatrix, colorCorrectionRgba, DEFAULT_COLOR);
        ...
        float yOffset = 0.02f;
        noseMatrix[13] += yOffset;

        float zOffset = 0.02f;
        noseMatrix[14] += zOffset;
        ...
        /*Pose noseTipPose = face.getRegionPose(RegionType.NOSE_TIP);

// Extract translation components
        float[] noseTranslation = new float[3];
        noseTipPose.getTranslation(noseTranslation, 0);

// Adjust the translation to position the specs around the eyes
        float xOffset = 0.0f; // Adjust as needed
        float yOffset = 0.1f; // Adjust as needed
        float zOffset = -0.1f; // Adjust as needed

// Apply the offset to the translation
        noseTranslation[0] += xOffset;
        noseTranslation[1] += yOffset;
        noseTranslation[2] += zOffset;

        float[] translatedMatrix = new float[16];
        Matrix.setIdentityM(translatedMatrix, 0);
        Matrix.translateM(translatedMatrix, 0, noseTranslation[0], noseTranslation[1], noseTranslation[2]);

// Apply the scale factor to the model matrix
        float[] modelMatrix1 = new float[16];
        face.getCenterPose().toMatrix(modelMatrix1, 0);
        Matrix.scaleM(modelMatrix1, 0, scaleFactor, scaleFactor, scaleFactor);

        float[] finalModelMatrix = new float[16];
        Matrix.multiplyMM(finalModelMatrix, 0, modelMatrix1, 0, translatedMatrix, 0);
        noseObject.draw(viewMatrix, finalModelMatrix, colorCorrectionRgba, DEFAULT_COLOR);*/
      }
    } catch (Throwable t) {
